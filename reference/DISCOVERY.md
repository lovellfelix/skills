...
- Optional `personal_machine_only: true` keeps a skill disabled unless the local-overlay flag (`~/.overlay/local/.enabled`) is absent — no allowlist needed
...
3. If the skill is local-overlay-gated, confirm `~/.overlay/local/.enabled` exists (or override flag path).
