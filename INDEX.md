...
| **lean-ctx** | `portable/lean-ctx/` | Context Engineering for AI Agents — 84 MCP tools, 10 read modes, 95+ shell patterns, tree-sitter AST for 27 languages. Compresses LLM context by up to 99%. Use when reading files, running shell commands, searching code, or exploring directories. Auto-installs if not present. |
...
| **pass-secrets-management** | `portable/pass-secrets-management/` | Use when agents need to retrieve or store secrets/passwords/credentials backed by the pass CLI, or when a human operator is provisioning/rotating credentials via pass-run.sh/pass-store.sh. |
