...
* `store_api_spec` - Store full OpenAPI spec (JSON/YAML)
* `get_api_endpoints` - Query endpoints by path pattern, method, or tag
* `get_api_schema` - Get request/response schema for an endpoint
* `get_api_endpoint_detail` - Full endpoint details with parameters
* `list_api_specs` - List all stored API specs with versions
* `search_api_endpoints` - Full-text search across endpoints
* `delete_api_spec` - Remove outdated API spec
...
get_api_endpoints --spec_id "user-service" --path_pattern "/users%"
get_api_schema --spec_id "user-service" --schema_name "CreateUserRequest"
